...
describe("Authentication", () => {
  test("User is able to sign up only once", async () => {
    const username = "ajay" + Math.random();
    ...
    console.log("updatedResponse 1 ",updatedResponse.status);
  ...
  test("Signup request fails if username is empty", async () => {
    ...
    console.log("response 3",response.status);
  ...
  test("Sign up succeeds if the username and password are correct ", async () => {
    ...
    console.log("response 4",response.status);
    expect(response.status).toBe(200);
    expect(response.data.token).toBeDefined();
  });
  test("signin failed when username and password are incorrect", async () => {
    const username = "ajay" + Math.random();
    const password = "123456";
    await axios.post(`${BACKEND_URL}/api/v1/signup`, {
      username,
    ...
    console.log("response 5",response.status);
    expect(response.status).toBe(400);
  });
});
...
//       }
//     );
//     console.log("avatarId response " + avatarResponse.data.avatarId);
//     avatarId = avatarResponse.data.avatarId;
//   });
//   test("User can not update meta data with wrong avatar id", async () => {
...
//     expect(response.status).toBe(400);
//   });
//   test("User can update meta data with correct avatar id", async () => {
//     const response = await axios.post(
//       `${BACKEND_URL}/api/v1/user/metadata`,
//       {
//         avatarId,
...
//     expect(response.status).toBe(200);
//   });
//   test("User is not able to update the metadat without auth header", async () => {
